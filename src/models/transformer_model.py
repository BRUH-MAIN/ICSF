...
class TransformerNLUWithCLS(nn.Module):
    """
    Variant with [CLS] token for intent classification.
    Adds a special CLS token at the beginning of each sequence.
    """
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        pad_idx: int = 0,
        cls_idx: int = 2  # Assuming [CLS] is at index 2
    ):
        super(TransformerNLUWithCLS, self).__init__()
        
        self.d_model = d_model
        self.pad_idx = pad_idx
        self.cls_idx = cls_idx
        
        # Token embedding (+1 for CLS token if not in vocab)
        self.embedding = nn.Embedding(
            vocab_size, 
            d_model, 
            padding_idx=pad_idx
        ...
        # Positional encoding
        self.positional_encoding = PositionalEncoding(d_model, max_len + 1, dropout)
        ...
        # Intent classification head (from CLS token)
        self.intent_classifier = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(d_model, d_model // 2),
            nn.ReLU(),
    ...
    def forward(
        ...
        """
        Forward pass with CLS token prepended.
        """
        batch_size, seq_len = input_ids.shape
        ...
        embedded = torch.cat([cls_tokens, embedded], dim=1)  # (batch, seq_len + 1, d_model)
        ...
        # Slot classification (exclude CLS position)
        slot_output = encoder_output[:, 1:, :]  # (batch, seq_len, d_model)
        slot_logits = self.slot_classifier(slot_output)  # (batch, seq_len, num_slots)
